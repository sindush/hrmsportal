...
fdescribe('SharedtableComponent', () => {
  ...
  let mockUtilityService: any;  // ✅ Use 'any' — plain object mock
  let mockSnackbarService: jasmine.SpyObj<SnackbarService>;
  let mockRouter: jasmine.SpyObj<Router>;
  let mockSpinnerService: jasmine.SpyObj<SpinnerService>;

  let dialogStatusSubject: Subject<boolean>; // ✅ Accessible across all tests

  beforeEach(async () => {

    // ✅ Fresh Subject before each test
    ...
    // ✅ Plain object — guarantees getDialogStatus is defined as a real Subject
    mockUtilityService = {
      getName: jasmine.createSpy('getName'),
      getDialogStatus: dialogStatusSubject,         // direct Subject assignment
      setEmployeeData: new Subject<any>()           // also used in component
    };

    // ✅ dialog.open() must return an object with close() and afterClosed()
    //    because closeDialog() calls dialogRef.close()
    mockDialog = {
    ...
    fixture.detectChanges(); // ✅ ngOnInit runs here — Subject is already set
  });
  ...
  it('should call closeDialog when getDialogStatus emits true', () => {
    ...
    // ✅ Emit directly on the shared Subject reference
    dialogStatusSubject.next(true);
